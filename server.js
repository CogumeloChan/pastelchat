...
function publicUser(u){return {username:u.username,displayName:u.display_name,description:u.description||'',nameColor:u.name_color||'#d44e82',online:!!sockets.get(u.username),avatarId:u.avatar_image_id||null}}
function broadcast(o){for(const ws of sockets.values())send(ws,o)}
async function notifyProfile(username){const r=await pool.query("SELECT username,display_name,description,name_color,avatar_image_id FROM users WHERE username=$1",[username]);if(r.rows[0])broadcast({type:'profile-update',user:publicUser(r.rows[0])})}
...
app.get("/api/me",async(req,res)=>{const a=auth(req);if(!a)return res.status(401).end();const r=await pool.query("SELECT id,username,display_name,description,name_color,avatar_image_id FROM users WHERE id=$1",[a.id]);if(!r.rows[0])return res.status(401).end();res.json(publicUser(r.rows[0]))});
app.get("/api/users/:username",async(req,res)=>{const r=await pool.query("SELECT id,username,display_name,description,name_color,avatar_image_id FROM users WHERE username=$1",[clean(req.params.username)]);if(!r.rows[0])return res.status(404).json({error:"Usuário não encontrado"});res.json(publicUser(r.rows[0]))});
...
app.get("/api/friends",async(req,res)=>{const a=auth(req);if(!a)return res.status(401).end();const r=await pool.query("SELECT u.username,u.display_name,u.description,u.name_color,u.avatar_image_id,f.nickname FROM friendships f JOIN users u ON u.id=f.friend_id WHERE f.user_id=$1 AND f.status='accepted' ORDER BY u.username",[a.id]);res.json(r.rows.map(u=>({...publicUser(u),nickname:u.nickname||''})))});
...
app.get("/api/friend-requests",async(req,res)=>{const a=auth(req);if(!a)return res.status(401).end();const r=await pool.query("SELECT u.username,u.display_name,u.description,u.name_color,u.avatar_image_id FROM friendships f JOIN users u ON u.id=f.user_id WHERE f.friend_id=$1 AND f.status='pending' ORDER BY f.created_at",[a.id]);res.json(r.rows.map(publicUser))});
