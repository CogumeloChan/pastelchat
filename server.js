...
app.get("/api/friends",async(req,res)=>{const a=auth(req);if(!a)return res.status(401).end();const r=await pool.query("SELECT u.username,u.display_name,u.description,u.name_color,u.avatar_image_id FROM friendships f JOIN users u ON u.id=f.friend_id WHERE f.user_id=$1 AND f.status='accepted' ORDER BY u.username",[a.id]);res.json(r.rows.map(publicUser))});
