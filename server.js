...
ALTER TABLE users ADD COLUMN IF NOT EXISTS banner_position_x NUMERIC NOT NULL DEFAULT 50;
ALTER TABLE users ADD COLUMN IF NOT EXISTS banner_position_y NUMERIC NOT NULL DEFAULT 50;
ALTER TABLE users ADD COLUMN IF NOT EXISTS banner_zoom NUMERIC NOT NULL DEFAULT 1;
...
function publicUser(u){const expired=u.status_expires_at && new Date(u.status_expires_at)<=new Date();return {username:u.username,displayName:u.display_name,description:u.description||'',nameColor:u.name_color||'#d44e82',online:!!sockets.get(u.username),avatarId:u.avatar_image_id||null,status:expired?'':(u.status_text||''),statusExpiresAt:expired?null:(u.status_expires_at||null),bannerId:u.banner_image_id||null,bannerPositionX:Number(u.banner_position_x??50),bannerPositionY:Number(u.banner_position_y??50),bannerZoom:Number(u.banner_zoom??1)}}
function broadcast(o){for(const ws of sockets.values())send(ws,o)}
async function notifyProfile(username){const r=await pool.query("SELECT username,display_name,description,name_color,avatar_image_id,banner_image_id,status_text,status_expires_at,banner_position_x,banner_position_y,banner_zoom FROM users WHERE username=$1",[username]);if(r.rows[0])broadcast({type:'profile-update',user:publicUser(r.rows[0])})}
...
app.get("/api/friends",async(req,res)=>{const a=auth(req);if(!a)return res.status(401).end();const r=await pool.query("SELECT u.username,u.display_name,u.description,u.name_color,u.avatar_image_id,u.banner_image_id,u.banner_position_x,u.banner_position_y,u.banner_zoom,u.status_text,u.status_expires_at,f.nickname FROM friendships f JOIN users u ON u.id=f.friend_id WHERE f.user_id=$1 AND f.status='accepted' ORDER BY u.username",[a.id]);res.json(r.rows.map(u=>({...publicUser(u),nickname:u.nickname||''})))});
...
app.get("/api/friend-requests",async(req,res)=>{const a=auth(req);if(!a)return res.status(401).end();const r=await pool.query("SELECT u.username,u.display_name,u.description,u.name_color,u.avatar_image_id,u.banner_image_id,u.banner_position_x,u.banner_position_y,u.banner_zoom,u.status_text,u.status_expires_at FROM friendships f JOIN users u ON u.id=f.user_id WHERE f.friend_id=$1 AND f.status='pending' ORDER BY f.created_at",[a.id]);res.json(r.rows.map(publicUser))});
...
app.patch("/api/profile",async(req,res)=>{const a=auth(req);if(!a)return res.status(401).end();const displayName=String(req.body.displayName||"").trim().slice(0,40);const description=String(req.body.description||"").trim().slice(0,160);const nameColor=/^#[0-9a-fA-F]{6}$/.test(String(req.body.nameColor||""))?String(req.body.nameColor):"#d44e82";if(!displayName)return res.status(400).json({error:"Informe um nome."});const r=await pool.query("UPDATE users SET display_name=$1,description=$2,name_color=$3 WHERE id=$4 RETURNING username,display_name,description,name_color,avatar_image_id,banner_image_id,status_text,status_expires_at,banner_position_x,banner_position_y,banner_zoom",[displayName,description,nameColor,a.id]);const user=publicUser(r.rows[0]);broadcast({type:'profile-update',user});res.json(user)});
app.patch("/api/profile/status",async(req,res)=>{const a=auth(req);if(!a)return res.status(401).end();const status=String(req.body.status||"").trim().slice(0,120);let expires=null;if(status)expires=new Date(Date.now()+24*60*60*1000);const r=await pool.query("UPDATE users SET status_text=$1,status_expires_at=$2 WHERE id=$3 RETURNING username,display_name,description,name_color,avatar_image_id,banner_image_id,status_text,status_expires_at,banner_position_x,banner_position_y,banner_zoom",[status,expires,a.id]);const user=publicUser(r.rows[0]);broadcast({type:"profile-update",user});res.json(user)});
app.post("/api/profile/avatar",upload.single("image"),async(req,res)=>{const a=auth(req);if(!a)return res.status(401).end();if(!req.file||!req.file.mimetype.startsWith("image/"))return res.status(400).json({error:"Envie uma imagem."});const imageId=id();await pool.query("INSERT INTO images(id,user_id,mime_type,data) VALUES($1,$2,$3,$4)",[imageId,a.id,req.file.mimetype,req.file.buffer]);const r=await pool.query("UPDATE users SET avatar_image_id=$1 WHERE id=$2 RETURNING username,display_name,description,name_color,avatar_image_id,banner_image_id,status_text,status_expires_at,banner_position_x,banner_position_y,banner_zoom",[imageId,a.id]);const user=publicUser(r.rows[0]);broadcast({type:'profile-update',user});res.json({id:imageId,url:"/api/images/"+imageId,user})});
app.post("/api/profile/banner",upload.single("image"),async(req,res)=>{const a=auth(req);if(!a)return res.status(401).end();if(!req.file||!req.file.mimetype.startsWith("image/"))return res.status(400).json({error:"Envie uma imagem."});const imageId=id();await pool.query("INSERT INTO images(id,user_id,mime_type,data) VALUES($1,$2,$3,$4)",[imageId,a.id,req.file.mimetype,req.file.buffer]);const r=await pool.query("UPDATE users SET banner_image_id=$1,banner_position_x=50,banner_position_y=50,banner_zoom=1 WHERE id=$2 RETURNING username,display_name,description,name_color,avatar_image_id,banner_image_id,status_text,status_expires_at,banner_position_x,banner_position_y,banner_zoom",[imageId,a.id]);const user=publicUser(r.rows[0]);broadcast({type:'profile-update',user});res.json({id:imageId,url:"/api/images/"+imageId,user})});
app.patch("/api/profile/banner-settings",async(req,res)=>{const a=auth(req);if(!a)return res.status(401).end();const x=Math.max(0,Math.min(100,Number(req.body.x??50))),y=Math.max(0,Math.min(100,Number(req.body.y??50))),zoom=Math.max(1,Math.min(3,Number(req.body.zoom??1)));const r=await pool.query("UPDATE users SET banner_position_x=$1,banner_position_y=$2,banner_zoom=$3 WHERE id=$4 RETURNING username,display_name,description,name_color,avatar_image_id,banner_image_id,status_text,status_expires_at,banner_position_x,banner_position_y,banner_zoom",[x,y,zoom,a.id]);const user=publicUser(r.rows[0]);broadcast({type:"profile-update",user});res.json(user)});
...
app.get("/api/rooms/:room/messages",async(req,res)=>{const a=auth(req);if(!a)return res.status(401).end();const room=String(req.params.room).slice(0,100);const r=await pool.query(`SELECT m.id,u.username,u.display_name AS "displayName",u.description,u.name_color AS "nameColor",u.avatar_image_id AS "avatarId",m.text,m.image_id AS "imageId",m.reply_to_id AS "replyToId",m.deleted_at AS "deletedAt",m.created_at AS time,ru.display_name AS "replyToDisplayName",rm.text AS "replyToText",rm.image_id AS "replyToImageId",rm.deleted_at AS "replyToDeleted" FROM messages m LEFT JOIN users u ON u.id=m.user_id LEFT JOIN messages rm ON rm.id=m.reply_to_id LEFT JOIN users ru ON ru.id=rm.user_id WHERE m.room=$1 ORDER BY m.created_at DESC LIMIT 100`,[room]);res.json(r.rows.reverse())});
...
wss.on("connection",ws=>{
 ...
 ws.on("message",async raw=>{
  ...
  if(m.type==="room-chat"){const text=String(m.text||"").trim().slice(0,3000);if(!text||!ws.room)return;const r=await pool.query("SELECT id FROM users WHERE username=$1",[ws.username]);const rawReplyId=m.replyToId?String(m.replyToId):"";const replyId=/^[0-9a-f-]{36}$/i.test(rawReplyId)?rawReplyId:null;let reply=null;if(replyId){reply=(await pool.query("SELECT m.id,m.text,m.image_id AS \"imageId\",m.deleted_at AS \"deletedAt\",u.display_name AS \"displayName\" FROM messages m LEFT JOIN users u ON u.id=m.user_id WHERE m.id=$1 AND m.room=$2",[replyId,ws.room])).rows[0];if(!reply)return;}const sender=(await pool.query("SELECT username,display_name,description,name_color,avatar_image_id FROM users WHERE username=$1",[ws.username])).rows[0];const item={id:id(),room:ws.room,username:ws.username,displayName:sender?.display_name||ws.username,description:sender?.description||"",nameColor:sender?.name_color||"#d44e82",avatarId:sender?.avatar_image_id||null,text,replyToId:reply?.id||null,replyToText:reply?.text||"",replyToImageId:reply?.imageId||null,replyToDisplayName:reply?.displayName||"",replyToDeleted:!!reply?.deletedAt,time:new Date().toISOString()};await pool.query("INSERT INTO messages(id,room,user_id,text,reply_to_id) VALUES($1,$2,$3,$4,$5)",[item.id,ws.room,r.rows[0].id,text,item.replyToId]);for(const c of rooms.get(ws.room)||[])send(c,{type:"chat",...item});await notifyUnread(ws.room,ws.username,item)}
  if(m.type==="room-image"){if(!ws.room||!m.imageId)return;const r=await pool.query("SELECT id FROM users WHERE username=$1",[ws.username]);const sender=(await pool.query("SELECT username,display_name,description,name_color,avatar_image_id FROM users WHERE username=$1",[ws.username])).rows[0];const rawReplyId=m.replyToId?String(m.replyToId):"";const replyId=/^[0-9a-f-]{36}$/i.test(rawReplyId)?rawReplyId:null;let reply=null;if(replyId){reply=(await pool.query("SELECT m.id,m.text,m.image_id AS \"imageId\",m.deleted_at AS \"deletedAt\",u.display_name AS \"displayName\" FROM messages m LEFT JOIN users u ON u.id=m.user_id WHERE m.id=$1 AND m.room=$2",[replyId,ws.room])).rows[0];if(!reply)return;}const item={id:id(),room:ws.room,username:ws.username,displayName:sender?.display_name||ws.username,description:sender?.description||"",nameColor:sender?.name_color||"#d44e82",avatarId:sender?.avatar_image_id||null,text:"",imageId:m.imageId,replyToId:reply?.id||null,replyToText:reply?.text||"",replyToImageId:reply?.imageId||null,replyToDisplayName:reply?.displayName||"",replyToDeleted:!!reply?.deletedAt,time:new Date().toISOString()};await pool.query("INSERT INTO messages(id,room,user_id,text,image_id,reply_to_id) VALUES($1,$2,$3,$4,$5,$6)",[item.id,ws.room,r.rows[0].id,"[imagem]",m.imageId,item.replyToId]);for(const c of rooms.get(ws.room)||[])send(c,{type:"chat",...item});await notifyUnread(ws.room,ws.username,item);return}
  if(m.type==="delete-message"){const messageId=String(m.messageId||"");if(!/^[0-9a-f-]{36}$/i.test(messageId))return;const r=await pool.query("UPDATE messages SET deleted_at=NOW() WHERE id=$1 AND user_id=(SELECT id FROM users WHERE username=$2) AND room=$3 AND deleted_at IS NULL RETURNING id,room",[messageId,ws.username,ws.room]);if(r.rows[0]){for(const c of rooms.get(ws.room)||[])send(c,{type:"message-deleted",messageId,room:ws.room})}return}
